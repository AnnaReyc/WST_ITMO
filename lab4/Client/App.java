...
public class App
{
    ...
    public static void main(String[] args)
    {
        ...
        printToConsole(getAllPersons(client, null));
        printToConosle(getAllPersons(client, "Thomas", "Muller", 45, "ThomMuler", "Main", 1 ));
    }
	
	private static void printToConsole(List<Person> persons)
    {
        for (Person person : persons)
            System.out.println(person.name + " " + person.surnname + ' ' + person.username +
                   		      ' ' + person.age + ' ' + person.position + ' ' + person.status);
    }
